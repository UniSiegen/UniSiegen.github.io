...
impact using `sacct -u username`. You can use this knowledge to set up the
...
use `sacct -u username` to get statistics about `parallel-job.sh`.

```bash
[username@hpc-login02 ~]$ sacct -u username
```
...
```bash
[username@hpc-login02 ~]$ sacct -u username -l -j 347087
```
...
```bash
[username@hpc-login02 ~]$ sacct -u username -l -j 347087 | less -S
```
